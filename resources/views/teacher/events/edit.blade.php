    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="post" action="#" class="editlessonform" id="eventaddform">
                {{ csrf_field() }} @forelse($userEvents as $events)
                <input type="hidden" id="geteventid" value="{{$events->id}}">
                <div class="row">
                    <div class="col-md-4 form-group">
                        <label>Start Date</label>
                        @php $var = $events->start_date; $date1 = str_replace('-', '/', $var); $starts_on = date('d/m/Y', strtotime($date1)); $var = $events->end_date; $date = str_replace('-', '/', $var); $ends_on = date('d/m/Y', strtotime($date)); @endphp
                    ...
                    <div class="col-md-4 form-group checkbox-field">
                        <label>
                            ...
                            <input name="schoolday" class='noSchool' type="checkbox" value="Y" @if ($events->school_day=='Y')checked="checked" @endif > No School Day</label>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 form-group checkbox-field">
                        <label>
                            ...
                            <input name="privateevent" type="checkbox" value="Y" @if ($events->private_event=='Y')checked="checked" @endif > Private Event</label>
                    </div>
                </div>
                <div class="row">
                ...
                <div class="addDiv">
                    @if($events->add_day!='null') 
                    @php $add_day = json_decode($events->add_day);
                    @endphp 
                ...
                <div class="removeDiv">
                    @if($events->remove_day!='null') @php $remove_day = json_decode($events->remove_day);@endphp @foreach($remove_day as $remove_days)
                    <div class="added-dayinner removeDays">
                        <div class="added-dayinner addDays">
                            <input name="removeday[]" class="form-control datepicker input-fields" id="demo8" type="text" size="10" value="{{$remove_days}}"><i class="fa fa-close" aria-hidden="true"></i></div>
                    </div>
                    @endforeach @endif
                </div>
                ...
                <div class="filetable">
                    <table id="attachedFiles" class="attachedFiles">
                        <tbody>
                            ...
                            @php $attachment = json_decode($events->attachment);
                            @endphp @foreach($attachment as $attach)

                            <tr>
                                <td class="filename" id="selectedFile"><a href="#">{{ $attach }}</a></td>
                                <td>
                                    <label>
                                        <input type="hidden" id="check_file" name="attach[]" value="{{ $attach }}">
                                        <input type="checkbox">Private</label>
                                </td>
                                <td>
...
<div id="editeventmodal" class="modal fade in movemodalcontent" style="display: none;">
    <div class="modal-dialog"> 
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-body">
          ...
          <div class="button-group">
            <button class="renew-button editshiftLessons" data-dismiss="modal"> Shift Lessons</button>
            <button class="renew-button dontedittLessons" data-dismiss="modal"> Do not Shift Lessons</button>
