...
<div class=" class-page events-section assignment-section">
	<div class="container-fluid">
		<div class="col-sm-12">
			<div class="teacher-dash-action pt-5 list-contentbutton gradebutons">
				...
				<div class="btn-group">
					<button type="button" class="btn btn-primary bg-white border-2 border-theme add-comments popup-custom-show unitsbutton list-contentmainbuton" id="addAssignmentButton" ><i class="fa fa-plus" aria-hidden="true"></i><span class="">Add Assignment</span></button>
				</div>
				<div class="btn-group">
...
/* Add assignment data*/
  ...
  $("body").on('click','#save_assignment_data_button',function(e){
	tinyMCE.triggerSave();  
	e.preventDefault();  
   ...
   /*Submit scores*/
    $('body').on('submit','.editscore-form',function(e){
		...
		if(totalSum == 100){
			...
			var url  = "/teacher/assignments/scoreAdd";
			$.ajax({
			  type:'POST',
			  url: BASE_URL +'/teacher/assignments/scoreAdd',
			  data: formData,
			  dataType: 'json',
				...
				success:function(response){
					...
					if(types!=''){
						...
						$.each(types, function(i, field){
							//$('#unit').append('<option value="" >Select Unit</option><option value="'+inc+'" >'+field.title+'</option>');
						//inc++;
					...
					$('#assignmentType').append('<option value="" >Select Type</option>');
					$.each($.parseJSON(assignments), function(i, field){
						$('#assignmentType').append('<option value="'+field.title+'" >'+field.title+'</option>');
					});
				},
				error: function(response){
					console.log("error");
