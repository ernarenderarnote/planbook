    ...
    <div class="modal-content">
       <div class="modal-header">
          ...
          <div class="actionright pull-right">
             <button id="edit_template_button" class="actiondropbutton renew-button">Save</button>
             <a class="closebutton" data-dismiss="modal"><i class="fa fa-close" aria-hidden="true"></i></a> 
          </div>
       </div>
       <div class="modal-body">
          <form method="post" id ="template_edit_form" action="#" class="addtemplateform">
            {{csrf_field()}}
            <input type="hidden" id="assessment_id" name="unit_id" value="{{ $template->id }}"/>
             <div class="row">
                ...
                @php
                      $var = $template->starts_on;
                      $date1 = str_replace('-', '/', $var);
                      $starts_on =  date('d/m/Y', strtotime($date1));

                      $var = $template->ends_on;
                      $date = str_replace('-', '/', $var);
                      $ends_on =  date('d/m/Y', strtotime($date));
                    @endphp
                <div class="col-md-4 form-group">
                   <label>Start On</label>
                   <input class="form-control input-fields datepicker" id="starts_on" name="starts_on" value="{{ $starts_on }}" type="text">
                </div>
             ...
             <div class="row">
                ...
                <div class="col-md-4 form-group">
                   <label>End On</label>
                   <input class="form-control input-fields datepicker" id="ends_on" name="ends_on" value="{{ $ends_on }}" type="text">
                </div>
                <div class="col-md-4 form-group checkbox-field">
                   <label>
             ...
             <div class="Description-memorial ">
                <p>Template</p>
                <textarea class="editorMce" name="description" placeholder="Write Somehting">{{ $template->description }}</textarea>
             </div>
          </form>
       </div>
    </div>
